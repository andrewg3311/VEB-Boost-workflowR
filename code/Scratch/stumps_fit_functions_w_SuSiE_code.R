...
weighted_SER = function(X, Y, sigma2, init = list(V = NULL)) {
  ...
  return(list(KL_div = KL_div, alpha = alpha, mu = mu, sigma2_post = sigma2_post, intercept = intercept, V = V, X_avg = X_avg, Y_avg = Y_avg))
}
...
predFn = function(X_new, currentFit, moment = c(1, 2)) {
  ...
  if (1 %in% moment) {
    res = c(res, list(mu1 = currentFit$intercept + compute_Xb(X_new, beta_post_1)))
  }
  if (2 %in% moment) {
    beta_post_2 = currentFit$alpha * (currentFit$sigma2_post + currentFit$mu^2)
    res = c(res, list(mu2 = currentFit$Y_avg^2 + 2*currentFit$Y_avg*(compute_Xb(X_new, beta_post_1) - sum(currentFit$X_avg * beta_post_1)) + compute_X2b(X_new, beta_post_2, currentFit$X_avg)))
  }
  return(res)
}
